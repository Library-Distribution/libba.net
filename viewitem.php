	...
	<body>
		...
		<div id="page-content">
			<?php
				if (!isset($id))
				{
					...
					foreach ($items as $item)
					{
						...
						echo "<li id='item{$item['id']}'><a class='item' href='?id={$item['id']}'>{$item['name']}</a> (v{$item['version']}) by <a class='userlink' href='users/{$item['user']['name']}'>{$item['user']['name']}</a></li>";
			...
			<table>
				<tr>
					<td>Uploaded by:</td>
					<td><a href="users/<?php echo $item['user']['name']; ?>"><?php echo $item['user']['name']; ?></a></td>
				</tr>
				<tr>
					<td>Uploaded:</td>
